...
export const HexColor = Color.extend({
  ...
  addCommands() {
    return {
      ...this.parent?.(),
      setBgColor: (backgroundColor) => ({ chain }) => chain().setMark('textStyle', { backgroundColor }).run(),
      unsetBgColor: () => ({ chain }) => chain().setMark('textStyle', { backgroundColor: null }).removeEmptyTextStyle().run(),
    }
  },
